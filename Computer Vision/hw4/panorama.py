...
def harris_corners(img, window_size=3, k=0.04):
    ...
    # convolve() : array is convolved with window as kernel
    # products of derivates/ squared
...
def simple_descriptor(patch):
    ...
    # normalizing the image values into std & then flattening it into 1D array
    std = np.std(patch)
    #  If a denominator is zero, divide by 1 instead
    if std == 0:
        feature = ((patch - np.mean(patch)) / 1).flatten()
    else:
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    # obtaining the indices of the minimum values along an axis
    miniValIndex = np.argmin(dists, axis = 1).reshape(-1, 1)
    arrangedKeypoints = np.arange(N).reshape(-1, 1)
    matches = np.concatenate([arrangedKeypoints, miniValIndex], axis = 1)
    sortedArray = np.sort(dists, axis = 1)
    # return the matche if ratio of the distances < than the threshold
    matches = matches[sortedArray[:, 0] / sortedArray[:, 1] < threshold]
    ### END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        maxInliner = np.zeros(N, dtype = np.int32)

        # selecting random set of matches
        index = np.random.choice(N, n_samples, replace=False)
        pointA = matched1[index, :]
        pointB = matched2[index, :]

        H = np.linalg.lstsq(pointB, pointA, rcond=None)[0]

        H[:, 2] = np.array([0, 0, 1])
        maxInliner = np.linalg.norm(matched2.dot(H) - matched1, axis = 1) ** 2 < threshold
        currentInliner = np.sum(maxInliner)

        # Keep the largest set of inliers
        if currentInliner > n_inliers:
            max_inliers = maxInliner.copy()
            n_inliers = currentInliner

    # Re-computing least-squares estimate on all of the inliers
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
        for j in range(cols):
            hist, bin_edges = np.histogram(a = theta_cells[i, j], bins = n_bins, range=(0, 180), weights = G_cells[i, j])
            cells[i, j] = hist

    # Flatten block of histograms into feature vector
    # & normalize flattened block
    cells = simple_descriptor(cells)
    block = cells.flatten()
...
def linear_blend(img1_warped, img2_warped):
    ...
    if left_margin < right_margin:
        ...
        # defining a weight matrix for image 1
        ...
        # defining a weight matrix for image 2
    ...
    # applying the weight matrices to their corresponding images
    merged = weight1 * img1_warped + weight2 * img2_warped

    # combining the images   
    overlapped = img1_mask + img2_mask
    maxValue = np.maximum(overlapped, 1)
    merged = merged / maxValue
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    for i in range(len(imgs) - 1):
        ransacResult = ransac(keypoints[i], keypoints[i + 1], matches[i], threshold = 1)
